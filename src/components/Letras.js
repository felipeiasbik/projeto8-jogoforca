...
function Letra({habLetra, disabled, valor, letraClicada, selecionados}){
    return (
        <div className={habLetra === false ? (selecionados.includes(valor) ? "letra" : "letra letra-habilitada") : "letra"} onClick={() => letraClicada(valor)} disabled={selecionados.includes(valor) && disabled} >{valor}</div>
